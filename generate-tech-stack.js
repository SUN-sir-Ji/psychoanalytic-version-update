  ...
  const dataRows = rows.map(row =>
    new TableRow({
      children: row.map((cellText, i) =>
        new TableCell({
          children: [new Paragraph({
            children: [new TextRun({ text: cellText, size: 21, font: "Arial" })],
          })],
          width: { size: colWidth, type: WidthType.DXA },
          margins: { top: 60, bottom: 60, left: 120, right: 120 },
          borders: { top: thinBorder, bottom: thinBorder, left: thinBorder, right: thinBorder },
          shading: i % 2 === 0 ? { fill: "F2F2F2", type: ShadingType.CLEAR } : undefined,
        })
      ),
    })
  );
    ...
    children: [
      ...
      makeTable(
        ...
        [
          ...
          ["表单处理", "React Hook Form 7.68 + Zod 4.3（校验）"],
      ...
      makeTable(
        ["技术类别", "技术名称及版本"],
        [
          ["用户认证", "PyJWT 2.8 + passlib（Argon2 / bcrypt 密码哈希）"],
          ["配置管理", "Pydantic Settings 2.2"],
          ["异步 HTTP", "httpx 0.25"],
          ["邮件发送", "emails 0.6 + Jinja2 3.1"],
          ["错误监控", "Sentry SDK 2.0"],
        ]
      ),
      ...
      makeTable(
        ...
        [
          ...
          ["暗色/亮色主题", "next-themes + CSS 变量（oklch 色彩空间）"],
        ]
      ),
      ...
      new Paragraph({
        children: [new TextRun({
          text: "注：本文档由项目代码自动整理生成，技术版本号为项目实际依赖版本。",
